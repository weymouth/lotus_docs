...
def plot_hist(pdf,name,label):
    ...
    plt.ylim([mn,mx])
    tm = df.time.mean()
    late = df.query('time>@tm')
    mean,mad = late[name].mean(), 1.5748*late[name].mad()
...
with PdfPages('history.pdf') as pdf:
    plot_hist(pdf,name='Fx',label=r'$F_{Xp}$')
    plot_hist(pdf,name='Fy',label=r'$F_{Yp}$')
    plot_hist(pdf,name='Cp',label=r'$C_{P}$')
